...
class TsCallFlowExtractor(BaseCallFlowExtractor):
    """
    Extracts function call flows from TypeScript/JavaScript code.
    ...
    Example:
        ...
        >>> entries = extractor.list_entry_points(Path("app.ts"))
        >>> graph = extractor.extract(Path("app.ts"), "main", max_depth=5)
    """
    ...
    def __init__(self, root_path: Optional[Path] = None, symbol_index=None) -> None:
        """Initialize the extractor with optional project root and symbol index."""
        super().__init__(root_path, symbol_index)
        # TS/TSX require separate language variants
        self._ts_language: Any = None
        self._tsx_language: Any = None

    @property
    def decision_types(self) -> Set[str]:
        """TypeScript/JavaScript decision point node types for complexity calculation."""
        return {
            "if_statement",
            "for_statement",
            "for_in_statement",
    ...
    def builtin_functions(self) -> Set[str]:
        """JavaScript/TypeScript builtin functions to ignore."""
        return JS_BUILTIN_FUNCTIONS

    # ─────────────────────────────────────────────────────────────
    # Override tree-sitter initialization for TS/TSX handling
    # ─────────────────────────────────────────────────────────────
    ...
    def _ensure_parser(self, is_tsx: bool = False) -> bool:
        """Ensure parser is initialized with correct language (TS or TSX)."""
    ...
    # ─────────────────────────────────────────────────────────────
    # Override complexity calculation for JS-specific patterns
    # ─────────────────────────────────────────────────────────────
    ...
    def _calculate_complexity(self, func_node: Any) -> int:
        ...
        Extended for TypeScript/JavaScript:
        - Counts && and || operators as decision points
        """
        count = 0
        to_visit = [func_node]
