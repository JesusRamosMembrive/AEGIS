...
def mock_state(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "code_map.state.ProjectScanner", lambda *args, **kwargs: mock_scanner
    )
...
async def test_run_similarity_bg_updates_state(mock_state):
    ...
    with patch("code_map.state.is_available", return_value=True), patch(
        "code_map.state.analyze_similarity", return_value=mock_report
    ) as mock_analyze, patch(
        "code_map.state.report_to_dict", return_value=mock_report_dict
    ):
...
async def test_perform_full_scan_triggers_similarity(mock_state):
    ...
    with patch("code_map.state.is_available", return_value=True), patch.object(
        mock_state, "run_similarity_bg", new_callable=AsyncMock
    ) as mock_run_bg:
