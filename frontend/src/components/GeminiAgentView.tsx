 ...
 * Gemini Agent View
 *
 * UI for interacting with Gemini Code via JSON streaming mode.
 * Replaces the TUI-based terminal with a structured, custom-rendered interface.
 */

import { useEffect, useRef, useState, useCallback } from "react";
import { useGeminiSessionStore, PERMISSION_MODES, PERMISSION_MODE_LABELS, PERMISSION_MODE_DESCRIPTIONS, type PermissionMode } from "../stores/geminiSessionStore";
...
export function GeminiAgentView() {
  ...
  const getWsUrl = useCallback(() => {
    ...
    return `${wsBase}/api/terminal/ws/gemini-agent`;
  }, [backendUrl]);
  ...
  useEffect(() => {
    if (!connected && !connecting) {
      ...
      console.log("[GeminiAgent] Connecting to:", wsUrl);
      connect(wsUrl);
    }
  ...
  const handleLoadSession = useCallback(
    (loadedMessages: ClaudeMessage[]) => {
      ...
      useGeminiSessionStore.setState({ messages: loadedMessages });
    },
    [clearMessages]
  );
  ...
  const handleAddPermission = useCallback(
    async (toolName: string) => {
      ...
      try {
        ...
        useGeminiSessionStore.setState((state) => ({
  ...
  return (
    <div
      ...
      role="main"
      aria-label="Gemini Agent Interface"
    >
            ...
            {running && (
              ...
              >
                <div className="thinking-content">
                  ...
                  <span className="thinking-text">Gemini is thinking...</span>
              ...
              <div className="execute-plan-banner" role="status">
                <div className="plan-banner-content">
                  ...
                  <span className="plan-banner-text">
                    Gemini described changes above. Click "Execute Plan" to apply them automatically.
                  </span>
                </div>
                <button
  ...
  return (
    <div className="claude-header">
      <div className="claude-header-left">
        <h2 className="claude-title">Gemini Agent</h2>
        ...
        <button
          ...
          title={
            continueSession
              ? "Continue mode: ON - Will resume previous Gemini session"
...
function EmptyState() {
  return (
    <div className="claude-empty">
      ...
      <p>Type a message below to start interacting with Gemini Code.</p>
      <p className="empty-hint">
        Gemini can read files, run commands, write code, and more.
      </p>
    </div>
  );
}
...
function MessageItem({ message }: MessageItemProps) {
  ...
  switch (message.type) {
    case "text":
      ...
      return (
        <div className="message message-text" role="listitem" aria-label="Gemini response">
